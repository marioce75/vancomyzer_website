    ...
    <Box dir={dir}>
      ...
      <Box sx={{ display:'flex', alignItems:'center', gap:1, mb:2, flexWrap:'wrap' }}>
        ...
        <Button size="small" variant="contained" color="primary" disabled={!apiOnline||loading} onClick={async ()=>{
          try{
            const data = await optimizeDose({ patient: { ...patient, mic: Number(patient.mic||1), population: 'pediatric' }, regimen: { dose_mg: doseMg, interval_hours: regimen.interval_hours, infusion_minutes: regimen.infusion_minutes }, target: targets.pediatric });
