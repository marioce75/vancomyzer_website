...
export function relevantRegionalEvidence(p: Pick<Evidence, "title" | "body_text">): boolean {
  const text = `${p.title} ${p.body_text ?? ""}`;
  return /vancomycin|vancomicina/i.test(text) && /pharmacokinetic|Bayesian|\bAUC\b|therapeutic drug monitor|area under the curve|dosing/i.test(text);
}
export function selectEvidence(posts: Evidence[]): Evidence[] {
  const bySource = new Map<string, Evidence[]>();
  const urls = new Set<string>();
  for (const p of posts) {
    if (p.source === "europepmc" && !relevantRegionalEvidence(p)) continue;
...
export function parseReport(response: string, evidence: Evidence[]): AnalystReport {
  ...
  if (/(?:regulatory|FDA)\s+(?:clearance|approval)(?:\s+\w+){0,3}\s+pending|pending\s+(?:regulatory|FDA)\s+(?:clearance|approval)/i.test(result.data.executive_brief)) {
    throw new AnalystError("invalid_response", "The report implies an unconfirmed regulatory application. No report was saved.");
  }
